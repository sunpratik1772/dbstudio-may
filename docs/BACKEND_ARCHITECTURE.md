...
5. **Generated artifacts are checked in.** `contracts/node_contracts.json`
   and `frontend/src/nodes/generated.ts` are regenerated by one script
   and committed. This means the frontend has no Python coupling at
   runtime.
...
The runner also powers `run_workflow_stream(...)` which yields SSE-shaped
events (`node_start`, `node_end`, `node_error`, `progress`, `done`).
...
6. **Generated artifacts** (`contracts/node_contracts.json`,
   `frontend/src/nodes/generated.ts`) are never hand-edited. Run
