...
4. **`frontend/src/nodes/index.ts` + `generated.ts`** — auto-generated
   from backend YAML. Never edit `generated.ts`; it's regenerated by
   `backend/scripts/gen_artifacts.py`. Every NodePanel category, every
   form field, every node colour comes from here.
...
7. **`frontend/src/components/NodeConfig/index.tsx`** — the config
   form. Auto-generated from the node's typed ParamSpec — you almost
   never edit this when adding a backend param.
...
| Add a node type to the palette               | Regenerate from backend; never edit `nodes/generated.ts` |
| Style a node                                 | Update the YAML on the backend, then regenerate       |
| Tweak a config form field                    | Backend YAML param spec → regenerate                  |
